# env/state.py
# Fixed:
# 1. trust_history logs UPDATED trust (after _update_trust), not old value
# 2. update() accepts system_trust parameter from info dict for accuracy
# 3. EpisodeState no longer double-updates trust (was updating internally
#    AND env was updating separately — now env is authoritative)
...
class EpisodeState:
    ...
    Fixed:
    - trust_history now appends AFTER _update_trust() (was before = stale value)
    - update() accepts optional system_trust from env info dict
      so EpisodeState trust matches env trust exactly (no drift)
    - Removed double-update: env manages trust authoritatively;
      EpisodeState uses the value passed in rather than recomputing
    """
    ...
    def update(
        ...
        event_name:     str   = None,
        hour:           int   = 0,
        system_trust:   float = None,   # FIX: accept from env info dict
    ):
        self.steps             += 1
        ...
        # FIX: if env passes its authoritative trust value, use it directly
        # instead of recomputing separately (which caused drift)
        if system_trust is not None:
            self.system_trust = float(np.clip(system_trust, 0.0, 1.0))
        else:
            # Fallback: compute trust independently (used when env doesn't pass it)
            self._update_trust(violations, complaints)

        # FIX: trust_history now appends AFTER trust is updated
        # (was appended before _update_trust in old code = stale value)
        ...
        self.hourly_log.append({
            ...
            "trust"      : round(self.system_trust, 3),  # FIX: now updated value
        })

    def _update_trust(self, violations: int, complaints: int):
        """
        Fallback trust update when env doesn't supply trust directly.
        Used by tasks that manage trust independently.
        """
        if violations > 0:
            self.system_trust *= (0.95 ** violations)
        elif complaints > 5:
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            "demand_satisfaction" : round(self.avg_demand_satisfaction(), 4),
            "avg_fairness"        : round(self.avg_fairness(), 4),
            "system_trust"        : round(self.system_trust, 4),
